...
def get_loss(config, device, reduction='mean'):
    """AI is creating summary for get_loss

    Args:
        config ([type]): [description]
        device ([type]): [description]
        reduction (str, optional): if more than one loss provided: how to aggregate them. Defaults to 'mean'.
    ...
    print("Loss function:" + loss_config['loss_function'])
    ...
    # Reg1 Loss -----------------------------------------------------------------------
    if loss_config['loss_function'] in ['reg_custom_loss']:
        alpha_regLoss = get_params_values(config["SOLVER"],"alpha_regLoss")
        print(loss_config['loss_function'] + "(alpha):", alpha_regLoss)
        return RegCustomLoss(alpha=alpha_regLoss)
    # Reg2 Loss -----------------------------------------------------------------------
    if loss_config['loss_function'] in ['resnet_loss']:
        alpha_regLoss = get_params_values(config["SOLVER"],"alpha_regLoss")
        print(loss_config['loss_function'] + "(alpha):", alpha_regLoss)
        return ResNetCustomLoss(alpha=alpha_regLoss)
...
class RegCustomLoss(torch.nn.Module):
        """alpha controls the mix between pure MSE (alpha=1) and pure STD (alpha=0)
        The idea of having STD in the loss is to promote the differences in the image, reduce the smoothing.
        """
        def __init__(self, alpha=1):
            super(RegCustomLoss, self).__init__()
            self.alpha=1

        def forward(self, output, target): # TODO separate for each channel
            mse = nn.functional.mse_loss(output, target, reduction="mean")
            # mae = nn.functional.l1_loss(output,target,reduction="mean")
            # mbe = torch.mean(torch.mean((target - output),dim=-1), dim=-1)
            #Contrast : idea is to match the same "variability level"
            std = (torch.std(output) - torch.std(target))**2
            # Assemble losses
            custom_loss = self.alpha * mse + (1 - self.alpha) * std
    
            return custom_loss

class ResNetCustomLoss(torch.nn.Module):
        """alpha controls the mix between pure MSE (alpha=1) and pure STD (alpha=0)
        The idea of having STD in the loss is to promote the differences in the image, reduce the smoothing.
        """
        def __init__(self, alpha=1):
            super(ResNetCustomLoss, self).__init__()
            self.alpha=1

        def forward(self, output, target): # TODO separate for each channel
            # target = target[...,target.shape[-2]//2,target.shape[-1]//2] # selecting the center pixel
            mse = nn.functional.mse_loss(output, target, reduction="mean")
            # mae = nn.functional.l1_loss(output,target,reduction="mean")
            # mbe = torch.mean(torch.mean((target - output),dim=-1), dim=-1)
            #Contrast : idea is to match the same "variability level"
